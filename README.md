...
- `079-telegram-userbot-platform-plugin` — a separate experimental, explicit opt-in Telegram userbot plugin through Telethon/MTProto, with profile-local locked sessions, deny-by-default allowlists enforced before gateway dispatch, no bot-style pairing or unsolicited home-channel onboarding for locally admitted senders, text/reply/edit, typing, optional length-aware human pacing with per-sender bypass IDs, bounded media caching, and native file delivery. See [docs/en/telegram-userbot.md](docs/en/telegram-userbot.md).
