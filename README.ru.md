...
Текущая поверхность `080` рассчитана на реальные IDE clients вроде RooCode: Chat Completions streaming отдаёт OpenAI-style SSE chunks, Codex Responses streams переводятся обратно в `chat.completion.chunk`, function/tool calls проходят как OpenAI `tool_calls`, inline image parts принимаются, а RooCode-style `reasoning_effort` мапится в backend; unsupported Codex sampling fields вроде `temperature` отфильтровываются до upstream 400.
