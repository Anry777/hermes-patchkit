...
Новости: PatchKit re-anchor'нут на Hermes Agent `v2026.5.29` / `0.15.1`. Старый Grok2API sidecar bridge теперь legacy fallback; для Grok/SuperGrok предпочитаем upstream-native `xai` / `xai-oauth`. См. [NEWS.md](NEWS.md).
...
python3 scripts/apply.py \
  ...
  --manifest manifests/upstream-v2026.5.29.yaml \
  --profile profiles/v2026.5.29-provider-proxy.yaml \
  --yes
```
...
python3 scripts/tui.py \
  --repo ~/.hermes/hermes-agent \
  --manifest manifests/upstream-v2026.5.29.yaml \
  --profile profiles/v2026.5.29-upstream-fixes.yaml
```
