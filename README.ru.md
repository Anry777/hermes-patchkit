...
Новости: PatchKit re-anchor'нут на Hermes Agent `v2026.8.3` / `0.20.0`. Текущий personal profile несёт только актуальный v0.20 overlay: shared root auth, суженный credential-pool recovery, оставшийся Telegram duplicate guard для first-chunk flood-control, MAX Bot API, MAX userbot и Telegram MTProto userbot plugins, provider proxy / Codex Responses proxy, configured WebSocket LSP, provider-plugin model switching, explicit media delivery safety, gateway auto-reset continuity и VibeMode provider plugin с Hermes User-Agent и provider-scoped фильтрацией `X-Stainless-*` для VibeMode OpenAI-wire endpoints. Более широкий Telegram rich/gating, SMTP_SSL, document media types, API-server fallback-model kwargs и старый Home Assistant config URL overlay retired там, где upstream v0.20 уже закрывает поведение. См. [NEWS.md](NEWS.md).

## Главный patch сейчас: Provider Proxy Gateway

`080-api-server-provider-proxy` — самый сильный patch в текущем наборе. Upstream Hermes API Server работает как agent endpoint, привязанный к профилю и его модели. PatchKit добавляет отдельные opt-in proxy contracts для разных задач: OpenAI-compatible gateway поверх provider/model routing с optional live model discovery и Responses-native Codex OAuth bridge для clients, которые напрямую говорят с `/v1/responses`.

В режиме `provider_proxy` `/v1/models` может показывать либо статический настроенный catalog, либо при `model_discovery: live` обновлять список моделей из выбранного backend'а и применять только regex allow/deny filters. `/v1/chat/completions` маршрутизирует запрос по `body.model` к выбранному provider/model target. Hermes не создаёт `AIAgent` для этих запросов: между клиентом и provider'ом нет SOUL prompt, tools, memory, sessions и agent run state. Для `openai-codex` patch держит OpenAI-compatible поверхность, а внутри использует Responses compatibility path.

Текущая Chat Completions поверхность `080` рассчитана на реальные IDE clients вроде RooCode: streaming отдаёт OpenAI-style SSE chunks, Codex Responses streams переводятся обратно в `chat.completion.chunk`, function/tool calls проходят как OpenAI `tool_calls`, inline image parts принимаются, RooCode-style `reasoning_effort` мапится в backend, unsupported Codex sampling fields вроде `temperature` фильтруются, а ChatGPT Codex calls получают обязательные для Cloudflare Codex origin headers. В тот же patch unit входит `codex_responses_proxy` для отдельного profile/port: он может live-discover Codex models с allow/deny filters и сохраняет Responses SSE events для Responses-native clients.
...
python3 scripts/tui.py \
  --repo ~/.hermes/hermes-agent \
  --manifest manifests/upstream-v2026.8.3.yaml \
  --profile profiles/v2026.8.3-upstream-fixes.yaml
```
...
- `scripts/grok2api_bridge.py` — legacy helper для dedicated Grok2API sidecar bridge; держим только как fallback, если native `xai` / `xai-oauth` не закрывает кейс;
...
- `v2026.8.3` / Hermes 0.20.0 re-anchor — active overlays закреплены на official release tag; `079` добавляет отдельный experimental Telegram MTProto userbot plugin.
...
- `079-telegram-userbot-platform-plugin` — отдельный experimental explicit opt-in Telegram userbot plugin через Telethon/MTProto: profile-local locked session, deny-by-default allowlist до gateway dispatch, без bot-style pairing/onboarding и известных Hermes slash-command/internal-error/approval UI для уже допущенных отправителей, с fail-closed approvals, text/reply/edit, typing, опциональным length-aware human pacing с исключениями по sender ID, bounded media cache и native file delivery. См. [docs/ru/telegram-userbot.md](docs/ru/telegram-userbot.md).
...
python3 scripts/update.py \
  --repo ~/.hermes/hermes-agent \
  --manifest manifests/upstream-v2026.8.3.yaml \
  --patch personal-overlay-v2026.8.3
```
...
- Telegram MTProto userbot plugin: [docs/ru/telegram-userbot.md](docs/ru/telegram-userbot.md)
