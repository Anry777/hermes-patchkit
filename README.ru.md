...
Новости: PatchKit re-anchor'нут на Hermes Agent `v2026.6.5` / `0.16.0`. Отдельный root-home media unit retired, потому что upstream поглотил основной fix. Старый Grok2API sidecar bridge теперь legacy fallback; для Grok/SuperGrok предпочитаем upstream-native `xai` / `xai-oauth`. См. [NEWS.md](NEWS.md).
