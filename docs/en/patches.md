...
Current release anchor: `manifests/upstream-v2026.5.29.yaml`. The release-specific patch files live under `patches/v2026.5.29/` and are checked against the official `NousResearch/hermes-agent` tag `v2026.5.29` / Hermes Agent `0.15.1`, not post-release `main`.
...
| `094-root-home-media-delivery` | exported | Allows root-run private gateways to deliver normal media artifacts from the current `/root` home without dropping `.zip` files before Telegram/Discord upload. | Keeps credential safety: `~/.ssh`, cloud/kube config dirs, Hermes `.env`, `auth.json`, `auth.lock`, `credentials`, and foreign `/root` paths remain blocked. |
