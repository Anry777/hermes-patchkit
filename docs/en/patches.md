...
| `062-codex-sdk-output-none-recovery` | exported | Catches TypeError from `stream.get_final_response()` when ChatGPT Codex backend sends `response.completed` with `output:null`, synthesizing the response from collected streaming events instead of crashing. | Works around `openai/lib/_parsing/_responses.py` line 61 bug (SDK ≤ 2.38.0). |
