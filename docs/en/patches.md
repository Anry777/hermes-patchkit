...
| `041-telegram-rich-flood-fallback` | exported | Keeps Telegram rich-message flood control from turning completed agent runs into user-visible delivery-failed notices. | If Bot API 10.1 `sendRichMessage` / rich `editMessageText` returns `RetryAfter` or `Flood control exceeded`, PatchKit treats it as a pre-delivery rejection and falls back to the legacy MarkdownV2 path, which already waits for Telegram’s retry window. Other transient rich transport errors still do not legacy-resend, preserving duplicate-safety. |
