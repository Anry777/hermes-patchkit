...
| `096-provider-plugin-model-switch` | exported | Makes model-provider plugins visible to `/model` and `/mode`. | Generic provider-plugin fix: `resolve_provider_full()`, provider labels, API-mode detection, and the no-args model picker now consult registered `ProviderProfile` entries, so plugin-backed providers such as `neurogate` do not need duplicate `config.yaml providers:` entries and do not fail with `Unknown provider`. |
