...
| `062-codex-sdk-output-none-recovery` | exported | Catches TypeError from Codex Responses stream iteration / `stream.get_final_response()` when ChatGPT Codex sends `response.completed` with `output:null`, recovering from streamed `output_item.done` / text deltas instead of crashing. | Preserves SDK terminal response metadata (`usage`, `model`, `id`) when available, handles explicit `output=None`, re-raises the original TypeError when there is nothing to recover, and carries focused runtime + auxiliary regression tests. |
