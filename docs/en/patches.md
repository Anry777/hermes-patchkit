...
| `100-vibemode-provider-plugin` | exported | Adds the VibeMode provider plugin, User-Agent override, and per-model endpoint metadata. | Sends `User-Agent: HermesAgent/1.0` for Responses compatibility; model availability remains live-discovered from `/v1/models`. |
...
| `041-telegram-rich-flood-fallback` | retired in `v2026.7.1` | Upstream v0.18 includes the Telegram rich/flood/overflow duplicate-safety path. | No standalone v0.18 patch file is shipped. |
