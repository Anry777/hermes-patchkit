...
| `096-provider-plugin-model-switch` | exported | Makes model-provider plugins visible to `/model`, `/mode`, and context resolution. | Generic provider-plugin fix: `resolve_provider_full()`, provider labels, API-mode detection, no-args model picker, and `get_model_context_length()` now consult registered `ProviderProfile` entries, so plugin-backed providers such as `neurogate` do not need duplicate `config.yaml providers:` entries, do not fail with `Unknown provider`, and can declare provider-enforced context windows (`neurogate` + `gpt-5.5` = 272,000 tokens). |
