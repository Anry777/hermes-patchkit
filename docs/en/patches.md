...
Current release anchor: `manifests/upstream-v2026.6.19.yaml`. The release-specific patch files live under `patches/v2026.6.19/` and are checked against the official `NousResearch/hermes-agent` tag `v2026.6.19` / Hermes Agent `0.17.0`, not post-release `main`.

## Available patch units

| Patch | Status | What it does | Notes |
|---|---|---|---|
| `010-cli-tui-idle-refresh-fix` | exported | Stops idle classic-CLI repaint from pulling the terminal viewport. | Reinstated for `v2026.6.19`: PatchKit sets `display.cli_refresh_interval` default to `0` because upstream 0.17 restored the 1 Hz idle refresh for status-bar freshness. Existing profile configs with an explicit `display.cli_refresh_interval: 1.0` override the patched default; set them to `0` with `hermes --profile <profile> config set display.cli_refresh_interval 0` or delete the explicit key. Set a positive interval only if you prefer the ticking idle status bar. |
...
| `030-credential-pool-recovery` | exported | Improves credential-pool recovery by tracking the active credential ID, preserving independent `openai-codex` OAuth pool entries, keeping invalid credentials out of cooldown recovery, and rotating round-robin entries only after leases are released. | Codex `auth add` now creates an independent pool credential instead of overwriting the singleton, refuses to spend a device-code login while stale Hermes auth writers can erase it, and verifies the new entry is really persisted before printing success. Runtime Codex resolution treats `credential_pool.openai-codex` as the canonical account list: when a pool slice exists, it does not bypass exhausted/dead/empty entries through legacy `providers.openai-codex.tokens`; the singleton remains only a migration fallback when the pool slice is absent. Successful round-robin requests also preselect the next credential inside the running agent, not only in persisted priority order. Runtime pool writes now merge only entries actually changed by the writer, so a long-lived proxy recording success for one credential cannot restore a sibling credential's `exhausted` status after `hermes auth reset`. Singleton re-auth syncs only true legacy aliases. Depends on `020-auth-profile-root-fallback`. |
| `040-telegram-free-response-target-gating` | exported | Prevents Telegram free-response group chats from hijacking messages explicitly addressed to another bot or user, supports release-pinned forum topic ownership via `telegram.allowed_threads`, exposes stable Telegram sender IDs in context/shared-group prefixes, and keeps `send_message(target="telegram")` in the current/home Telegram forum topic instead of dropping attachments into another topic. | Uses positive addressing: direct mentions/replies/wake words address this bot, but fresh explicit addressing to another target (`@other_bot`, `/cmd@other_bot`, `Name, ...`) wins over reply context; ambient free-response chat does not hijack unaddressed questions. In an explicitly allowed forum topic with `require_mention: false`, ordinary messages are accepted even when Telegram attaches reply metadata from another bot/service message. With `privacy.redact_pii: true`, sender IDs are hashed before prompt exposure. |
| `041-telegram-rich-flood-fallback` | exported | Keeps Telegram rich-message flood control and long-response overflow handling from turning completed agent runs into delivery-failed notices or duplicate visible chunks. | If Bot API 10.1 `sendRichMessage` / rich `editMessageText` returns `RetryAfter` or `Flood control exceeded`, PatchKit treats it as a pre-delivery rejection and falls back to the legacy MarkdownV2 path, which already waits for Telegram’s retry window. If a long final response is split across multiple Telegram messages, the stream consumer records that overflow delivery and skips the second finalize path; if a final formatting/finalize edit is flood-limited after the exact final text is already visible, Hermes suppresses the normal final-send path instead of posting a second answer bubble. Partial overflow failures are non-retryable so Hermes does not resend already-visible chunks. Other transient rich transport errors still do not legacy-resend, preserving duplicate-safety. |
...
| `061-codex-auxiliary-tool-role-flattening` | retired in `v2026.6.19` | Upstream 0.17 routes auxiliary Codex Responses calls through the shared converter, so raw `role="tool"` no longer leaks into Responses input. | No standalone PatchKit unit is carried forward for `v2026.6.19`. |
...
| `078-max-userbot-platform-plugin` | exported | Adds a separate experimental `max_userbot` platform plugin backed by MaxApiTeam/PyMax (`maxapi-python`) for user-account MAX access when official Bot API bot creation is unavailable. | Intentionally separate from `070` and from official MAX Bot API semantics. Stores PyMax sessions profile-locally, uses session locking, supports TCP/WebClient bootstrap paths, allowlists, text/reply/edit, media mapping, PyMax file delivery, progress edits, and inline approval payloads. This is an unofficial internal MAX API userbot path and needs operator risk acceptance plus real account SMS/QR bootstrap for live smoke. |
...
| `092-gateway-document-media-types` | exported | Syncs gateway document/media attachment types. | Keeps `.epf`/`.cfe` support for 1C as binary documents and fixes explicit `MEDIA:<path>` directives for supported artifacts such as `.md`, `.markdown`, `.json`, `.yaml`, `.html`: they are extracted and sent as native attachments instead of plain text; the broad `\S+` fallback is not restored. |
| `093-neurogate-provider-plugin` | exported | Adds NeuroGate as a bundled model-provider plugin. | The only provider id is `neurogate`; no aliases. `hermes auth add neurogate --type api-key` saves a credential-pool entry for `https://api.neurogate.space/v1`; runtime routing forces `codex_responses` so GPT-5.x / Responses-compatible endpoints do not fall back to Chat Completions. |
| `096-provider-plugin-model-switch` | exported | Makes model-provider plugins visible to `/model`, `/mode`, and context resolution. | Generic provider-plugin fix: `resolve_provider_full()`, provider labels, API-mode detection, no-args model picker, and `get_model_context_length()` now consult registered `ProviderProfile` entries, so plugin-backed providers such as `vibemode` do not need duplicate `config.yaml providers:` entries, do not fail with `Unknown provider`, and can declare provider-enforced context windows (`vibemode` + `gpt-5.5` = 272,000 tokens). |
| `100-vibemode-provider-plugin` | exported | Renames the local NeuroGate overlay to VibeMode and adds per-model endpoint metadata. | `hermes auth add vibemode --type api-key` stores credentials for `https://api.vibemod.pro/v1`. Model availability stays dynamic through `/v1/models`; the plugin only overlays known model properties: context windows, endpoint kind, capabilities, and preferred runtime (`chat_completions`, `codex_responses`, or `anthropic_messages`). The final runtime removes the old `neurogate` provider surface. |
| `097-gateway-explicit-media-delivery-safety` | exported | Makes gateway local-file delivery explicit by default. | `MEDIA:/path` and structured `artifacts=[...]` still send as native attachments after validation; bare `/absolute/path/file` remains text unless the operator explicitly enables `gateway.auto_upload_local_paths: true`. `gateway.strict` remains a separate validation layer. |
| `098-api-server-fallback-model-kwarg` | exported | Prevents the OpenAI-compatible API Server fallback path from passing `model` twice to `AIAgent`. | When provider fallback returns runtime kwargs with a fallback model, `_create_agent()` pops that model out, uses it as the effective model override, and keeps the rest of the runtime kwargs unchanged. This fixes `AIAgent() got multiple values for keyword argument 'model'` after primary-provider auth/quota failure. |
| `099-gateway-auto-reset-context-continuity` | exported | Keeps gateway chat context across idle/daily auto-reset. | The new session stores `parent_session_id` for the expired session, gateway rehydrates reset-parent transcripts automatically, and compression ancestry stays collapsed; manual `/new` and `/reset` remain fresh sessions. |
| `094-root-home-media-delivery` | retired in `v2026.6.5` | Upstream 0.16 now accepts the active `/root` home as a deliverable media location while keeping credential subdirectories and Hermes auth/env files blocked. | No standalone PatchKit unit is carried forward for `v2026.6.5`; the remaining `.epf`/`.cfe` and explicit-media parity work lives in `092`/`097`. |
| `080-api-server-provider-proxy` | exported | Adds opt-in raw provider proxy modes to the OpenAI-compatible API Server: `provider_proxy` keeps `/v1/models` as an explicit catalog and routes `/v1/chat/completions`; `codex_responses_proxy` exposes a Responses-native Codex OAuth bridge on `/v1/responses` without creating an `AIAgent`. | Generic upstream-candidate patch. Supports non-streaming and streaming Chat Completions for OpenAI-compatible providers, plus a compatibility path for `openai-codex`/Responses that adapts Responses streams into OpenAI Chat Completion SSE chunks, maps `reasoning_effort`, and filters ChatGPT Codex-rejected sampling params such as `temperature`. The Responses-native mode keeps Responses SSE events (`response.created`, `response.output_text.delta`, `response.completed`), can live-discover Codex models with allow/deny filters, forces ChatGPT Codex `store: false`, and records credential-pool success/exhaustion for the selected account. `/v1/runs` still fails closed as an unsupported operation. |
