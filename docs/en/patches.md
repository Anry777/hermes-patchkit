...
| `079-telegram-userbot-platform-plugin` | exported | Adds a separate experimental `telegram_userbot` platform plugin backed by Telethon/MTProto. | User-account path with profile-local locked sessions, deny-by-default allowlists that bypass bot-style pairing after local admission, suppressed unsolicited home-channel onboarding, and optional length-aware human pacing with per-sender bypass IDs; see [telegram-userbot.md](telegram-userbot.md). |
