...
| `077-max-markdown-formatting` | exported | Делает исходящие MAX-сообщения менее плоскими: text и captions по умолчанию уходят с official MAX Markdown formatting. | Зависит от `076-max-media-directive-safety`. Добавляет настройку `MAX_TEXT_FORMAT` (`markdown`, `html` или invalid/disabled values для отправки без formatting metadata), сохраняет explicit per-message metadata overrides и обновляет MAX prompt hint под аккуратный MAX-safe Markdown без code block вокруг `MEDIA:` lines. |
