...
| `096-provider-plugin-model-switch` | exported | Делает model-provider plugins видимыми для `/model` и `/mode`. | Общий fix для provider plugins: `resolve_provider_full()`, provider labels, API-mode detection и no-args model picker теперь читают зарегистрированные `ProviderProfile`, поэтому plugin-backed providers вроде `neurogate` не требуют дублирующего `config.yaml providers:` entry и не падают с `Unknown provider`. |
