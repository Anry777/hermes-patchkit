...
| `041-telegram-rich-flood-fallback` | exported | Не даёт Telegram rich-message flood control превращать завершённые agent runs в видимые пользователю delivery-failed notices. | Если Bot API 10.1 `sendRichMessage` / rich `editMessageText` возвращает `RetryAfter` или `Flood control exceeded`, PatchKit считает это pre-delivery отказом и откатывается на legacy MarkdownV2 path, который уже ждёт retry window Telegram. Другие transient rich transport errors по-прежнему не пересылаются legacy-путём, чтобы не создавать дубликаты. |
