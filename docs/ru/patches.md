...
| `100-vibemode-provider-plugin` | exported | Добавляет VibeMode provider plugin, User-Agent override и per-model endpoint metadata. | Отправляет `User-Agent: HermesAgent/1.0` для Responses compatibility; список моделей остаётся live-discovered через `/v1/models`. |
