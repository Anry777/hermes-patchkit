...
Текущий релизный якорь: `manifests/upstream-v2026.5.29.yaml`. Release-specific patch files лежат в `patches/v2026.5.29/` и проверяются против official tag `v2026.5.29` / Hermes Agent `0.15.1` из `NousResearch/hermes-agent`, а не против post-release `main`.
...
| `092-1c-document-types` | exported | Добавляет `.epf` и `.cfe` (внешняя обработка / расширение конфигурации 1C:Предприятие) в `SUPPORTED_DOCUMENT_TYPES` как `application/octet-stream`. | Бинарные артефакты; агент получает только кэшированный путь к файлу, без инлайн-рендеринга или исполнения. |
| `093-neurogate-provider-plugin` | exported | Добавляет NeuroGate как bundled model-provider plugin. | `hermes auth add neurogate --type api-key` сохраняет credential pool entry с `https://api.neurogate.space/v1`; aliases: `neurogate-space`, `neurogate-api`; runtime routing принудительно использует `codex_responses`, чтобы GPT-5.x/Responses-compatible endpoints не падали обратно в Chat Completions. |
| `094-root-home-media-delivery` | exported | Позволяет private gateway, запущенному от root, отправлять нормальные media artifacts из текущего `/root` home, не отбрасывая `.zip` до Telegram/Discord upload. | Credential safety сохраняется: `~/.ssh`, cloud/kube config dirs, Hermes `.env`, `auth.json`, `auth.lock`, `credentials` и чужие `/root` paths остаются заблокированы. |
