...
Текущий релизный якорь: `manifests/upstream-v2026.6.19.yaml`. Release-specific patch files лежат в `patches/v2026.6.19/` и проверяются против official tag `v2026.6.19` / Hermes Agent `0.17.0` из `NousResearch/hermes-agent`, а не против post-release `main`.

## Доступные patch units

| Patch | Статус | Что делает | Примечания |
|---|---|---|---|
| `010-cli-tui-idle-refresh-fix` | exported | Останавливает idle repaint classic CLI, который тянет terminal viewport. | Возвращён для `v2026.6.19`: PatchKit ставит default `display.cli_refresh_interval` в `0`, потому что upstream 0.17 снова включил 1 Hz idle refresh ради свежего status bar. Если в существующем profile config уже явно записано `display.cli_refresh_interval: 1.0`, это перекрывает patched default; поставь `0` командой `hermes --profile <profile> config set display.cli_refresh_interval 0` или удали явный ключ. Positive interval нужен только если сознательно хочешь ticking idle status bar. |
...
| `030-credential-pool-recovery` | exported | Улучшает credential-pool recovery: отслеживает active credential ID, сохраняет независимые `openai-codex` OAuth pool entries, не возвращает invalid credentials через cooldown recovery и откладывает round-robin rotation до release lease. | Codex `auth add` теперь создаёт независимый pool credential вместо перезаписи singleton, отказывается тратить device-code login при живых stale Hermes auth writers и проверяет, что новая запись реально сохранилась перед success. Runtime Codex resolution считает `credential_pool.openai-codex` canonical account list: при существующем pool он не обходит exhausted/dead/empty entries через legacy `providers.openai-codex.tokens`; singleton остаётся только migration fallback, когда pool slice отсутствует. Успешные round-robin requests также заранее переключают следующий credential внутри running agent, а не только меняют persisted priority order. Runtime pool writes теперь merge'ят только реально изменённые writer'ом entries, поэтому long-lived proxy, записывающий success по одному credential, не может вернуть соседнему credential `exhausted` после `hermes auth reset`. Singleton re-auth синхронизирует только настоящие legacy aliases. Зависит от `020-auth-profile-root-fallback`. |
...
| `041-telegram-rich-flood-fallback` | exported | Не даёт Telegram rich-message flood control и long-response overflow handling превращать завершённые agent runs в delivery-failed notices или дубли видимых chunks. | Если Bot API 10.1 `sendRichMessage` / rich `editMessageText` возвращает `RetryAfter` или `Flood control exceeded`, PatchKit считает это pre-delivery отказом и откатывается на legacy MarkdownV2 path, который уже ждёт retry window Telegram. Если длинный финальный ответ split-ится на несколько Telegram messages, stream consumer запоминает overflow delivery и пропускает второй finalize path; если финальный formatting/finalize edit упирается во flood-control после того, как точный финальный текст уже виден, Hermes подавляет обычную final-send дорожку и не постит второй answer bubble. Partial overflow failures non-retryable, поэтому Hermes не пересылает уже видимые chunks. Другие transient rich transport errors по-прежнему не пересылаются legacy-путём, чтобы не создавать дубликаты. |
...
| `061-codex-auxiliary-tool-role-flattening` | retired in `v2026.6.19` | Upstream 0.17 уже проводит auxiliary Codex Responses calls через shared converter, поэтому raw `role="tool"` больше не уходит в Responses input. | Отдельный PatchKit unit не переносится в `v2026.6.19`. |
...
| `078-max-userbot-platform-plugin` | exported | Добавляет отдельный experimental `max_userbot` platform plugin на базе MaxApiTeam/PyMax (`maxapi-python`) для доступа к MAX через user account, когда создание official Bot API bot недоступно. | Намеренно отдельно от `070` и official MAX Bot API semantics. Хранит PyMax sessions profile-local, использует session locking, поддерживает TCP/WebClient bootstrap paths, allowlists, text/reply/edit, media mapping, PyMax file delivery, progress edits и inline approval payloads. Это unofficial internal MAX API userbot path: для live smoke нужны operator risk acceptance и реальный SMS/QR bootstrap аккаунта. |
...
| `096-provider-plugin-model-switch` | exported | Делает model-provider plugins видимыми для `/model`, `/mode` и context resolution. | Общий fix для provider plugins: `resolve_provider_full()`, provider labels, API-mode detection, no-args model picker и `get_model_context_length()` теперь читают зарегистрированные `ProviderProfile`, поэтому plugin-backed providers вроде `vibemode` не требуют дублирующего `config.yaml providers:` entry, не падают с `Unknown provider` и могут объявлять provider-enforced context windows (`vibemode` + `gpt-5.5` = 272,000 tokens). |
| `100-vibemode-provider-plugin` | exported | Переименовывает local NeuroGate overlay в VibeMode и добавляет per-model endpoint metadata. | `hermes auth add vibemode --type api-key` сохраняет credential для `https://api.vibemod.pro/v1`. Список моделей остаётся динамическим через `/v1/models`; plugin только накладывает свойства известных моделей: context window, endpoint kind, capabilities и preferred runtime (`chat_completions`, `codex_responses` или `anthropic_messages`). В финальном runtime старый provider surface `neurogate` удалён. |
...
| `099-gateway-auto-reset-context-continuity` | exported | Сохраняет контекст gateway-диалога после idle/daily auto-reset. | Новая session получает `parent_session_id` на expired session, gateway rehydrate'ит reset-parent transcripts автоматически и не разворачивает compression ancestry; ручные `/new` и `/reset` остаются свежими сессиями. |
