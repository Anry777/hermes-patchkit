...
| `062-codex-sdk-output-none-recovery` | exported | Ловит TypeError из `stream.get_final_response()`, когда ChatGPT Codex backend шлёт `response.completed` с `output:null`, и синтезирует ответ из собранных streaming events вместо краша. | Обходит баг `openai/lib/_parsing/_responses.py` строка 61 (SDK ≤ 2.38.0). |
