...
| `062-codex-sdk-output-none-recovery` | exported | Ловит TypeError из Codex Responses stream iteration / `stream.get_final_response()`, когда ChatGPT Codex шлёт `response.completed` с `output:null`, и восстанавливает ответ из streamed `output_item.done` / text deltas вместо краша. | Сохраняет metadata SDK terminal response (`usage`, `model`, `id`), если она есть, обрабатывает явный `output=None`, re-raise-ит исходный TypeError, когда восстанавливать нечего, и содержит focused runtime + auxiliary regression tests. |
