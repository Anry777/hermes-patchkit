...
| `100-vibemode-provider-plugin` | exported | Добавляет VibeMode provider plugin, User-Agent propagation, Bearer auth для Messages и per-model endpoint metadata. | Отправляет `User-Agent: HermesAgent/1.0` через OpenAI-compatible и Anthropic Messages transports, не использует x-api-key auth для VibeMode Messages и не exhaust'ит credentials на Cloudflare/WAF 403 blocks; список моделей остаётся live-discovered через `/v1/models`. |
...
| `041-telegram-rich-flood-fallback` | retired in `v2026.7.1` | Upstream v0.18 включает Telegram rich/flood/overflow duplicate-safety path. | Отдельный v0.18 patch file не поставляется. |
