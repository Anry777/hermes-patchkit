...
| `041-telegram-rich-flood-fallback` | exported | Не даёт Telegram rich-message flood control и long-response overflow handling превращать завершённые agent runs в delivery-failed notices или дубли видимых chunks. | Если Bot API 10.1 `sendRichMessage` / rich `editMessageText` возвращает `RetryAfter` или `Flood control exceeded`, PatchKit считает это pre-delivery отказом и откатывается на legacy MarkdownV2 path, который уже ждёт retry window Telegram. Если длинный финальный ответ split-ится на несколько Telegram messages, stream consumer запоминает overflow delivery и пропускает второй finalize path; partial overflow failures non-retryable, поэтому Hermes не пересылает уже видимые chunks. Другие transient rich transport errors по-прежнему не пересылаются legacy-путём, чтобы не создавать дубликаты. |
