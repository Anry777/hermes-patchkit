...
Release patch должен чисто применяться к official `v2026.8.3`, а после apply должен проходить `git apply -R --check`.
