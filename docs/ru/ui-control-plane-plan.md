...
Follow-up semantic slice после `214`. Этот patch исправляет саму модель Overview: это system overview, а не страница выбранного профиля.

Реализованный состав:

- `/api/dashboard/overview` добавляет `system_summary` для top-level профилей, sessions, runtime, messaging adapters и memory;
- backend добавляет safe `memory` inventory по профилям и external banks: enabled/user-profile flags, provider, external provider, storage type, local profile counts, live Hindsight bank counts, byte usage и usage state без чтения/возврата содержимого памяти;
- profile rows получают compact memory metadata, чтобы Profiles table была inventory по всей системе;
- `selected_profile` остаётся backward-compatible field, но Overview UI больше не выносит его в hero/cards;
- Overview hero/copy говорит `System overview`, верхние cards теперь system-wide, а `Profile health` переименован в `Profiles`;
- sidebar status показывает общий session count, а не current-profile/total split;
- safety boundary остаётся metadata-only: без memory contents, session IDs, message bodies, prompts, tool args/results, env, auth, logs или secrets.

Статус: exported как PatchKit unit `215-dashboard-system-overview-semantics`; runtime commit `1e17b8776`, depends on `200`–`214`. Validation: RED source-contract/API tests сначала падали на отсутствующих `system_summary`/`memory`, затем focused dashboard suite passed; `npm run build`; focused ESLint for `OverviewPage.tsx`, `api.ts`, `SidebarStatusStrip.tsx`; `py_compile`; `git diff --check`; live smoke на `10.50.50.28:9119/overview?smoke=215-async` подтвердил system-wide Overview, `external_banks`/Hindsight live count (`infra`, 3620 records, `source=hindsight_api`) и отсутствие browser console errors.

Почему после `214`: после очистки adapter semantics стало видно, что Overview всё ещё моделировался вокруг selected profile. `215` возвращает странице роль общесистемной панели control plane.
